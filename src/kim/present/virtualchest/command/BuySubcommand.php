	...
	public function execute(CommandSender $sender, array $args = []) : bool{
		if($sender instanceof Player){
			...
			if($count >= $this->plugin->getMaxCount()){
				$sender->sendMessage($this->plugin->getLanguage()->translate("commands.virtualchest.buy.failure.max"));
				return true;
			}else{
				...
				if($price === null){
					$sender->sendMessage($this->plugin->getLanguage()->translate("commands.virtualchest.buy.failure.prevent"));
					return true;
				}elseif(!isset($this->checked[$playerName]) || (time() - $this->checked[$playerName]) > 10){
					$this->checked[$playerName] = time();
					$sender->sendMessage($this->plugin->getLanguage()->translate("commands.virtualchest.buy.check", [(string) $price]));
				}else{
					unset($this->checked[$playerName]);
					if($myMoney < $price){
						$sender->sendMessage($this->plugin->getLanguage()->translate("commands.virtualchest.buy.failure.money", [(string) $myMoney]));
					}else{
						...
						$sender->sendMessage($this->plugin->getLanguage()->translate("commands.virtualchest.buy.success", [(string) ($myMoney - $price)]));
					}
				}
			}
		}else{
